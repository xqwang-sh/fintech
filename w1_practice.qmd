...
2. **选择版本**：建议选择最新稳定版本（3.10、3.11 或 3.12）
3. **安装步骤**：
   - 下载适合你操作系统的安装程序
...
print("观察：从图上可以看出，面积越大，价格越高，呈现线性关系 ✓")
...
print(f"\n预测：95平米的房子价格约为 {predicted:.1f} 万元")

# ========== 第8步：评估模型 - 可视化拟合效果 ==========
# 画图对比：真实数据 vs 模型拟合的直线
plt.figure(figsize=(8, 5))
plt.scatter(areas, prices, s=100, alpha=0.7, color='blue', label='真实数据点')
plt.plot(areas, model.predict(X), color='red', linewidth=2, label='模型拟合直线')
