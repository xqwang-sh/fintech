...
#| warning: false
# 导入库：使用时需要加库名
import numpy as np       # 导入 numpy 库，通常缩写为 np
import pandas as pd      # 导入 pandas 库，通常缩写为 pd

# numpy 示例：数值计算和数组操作
# 注意：numpy数组 vs Python列表的区别
# - 列表：Python内置，灵活但计算慢
# - 数组：numpy专用，高效但类型固定
scores_list = [85, 90, 78, 92, 88]        # Python 列表
scores = np.array(scores_list)             # 转换为 numpy 数组
print("原始列表:", scores_list)
print("numpy数组:", scores)
print("平均分:", np.mean(scores))         # numpy 计算平均值，输出：86.6
print("最高分:", np.max(scores))          # 最高分，输出：92
print("最低分:", np.min(scores))          # 最低分，输出：78
...
df = pd.DataFrame(data)  # 创建 DataFrame（数据表格）
print("\n学生成绩表:")
print(df)               # 显示表格
print("\n平均成绩:", df['成绩'].mean())  # pandas 计算平均值
```

**练习题：** 使用 numpy 和 pandas 创建一个简单的学生成绩管理系统。

**要求：**

1. 使用 numpy 创建一个包含 5 个学生成绩的数组：`[85, 92, 78, 96, 88]`
2. 计算这些成绩的平均值、最高分和最低分
3. 使用 pandas 创建一个 DataFrame，包含学生的姓名和成绩
4. 显示 DataFrame 并计算平均成绩

**提示：**

- numpy 数组创建：`np.array([数值列表])`
- numpy 统计函数：`np.mean()`, `np.max()`, `np.min()`
- pandas DataFrame 创建：`pd.DataFrame({'列名': [数据]})`
- DataFrame 平均值：`df['列名'].mean()`
