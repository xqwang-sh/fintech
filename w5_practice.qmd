...
- 读取 W3 预处理后的数据
- 随机森林模型训练与超参数调优
- GBDT（LightGBM）模型训练与超参数调优
- 三模型对比：决策树 vs 随机森林 vs GBDT

**学习目标**：

- 理解随机森林的工作原理和优势
- 掌握 GBDT（梯度提升树）的基本概念
- 学会使用 GridSearchCV 进行超参数调优
- 比较不同集成学习方法的性能

**数据来源**：`titanic_data/features_processed.csv` 和 `titanic_data/labels_processed.csv`（来自 W4 的预处理结果）
...
**说明**：我们使用 W3 预处理后的数据，直接加载即可。数据已经完成：

- 缺失值处理
- 特征编码（Sex 标签编码，Embarked 独热编码）
- 特征选择

**注意**：数据分割的 random_state 需要与 W3 保持一致（random_state=42），这样才能使用相同的测试集进行公平对比。
...
print(f"训练集存活率: {y_train.mean().values[0]:.3f}")
print(f"测试集存活率: {y_test.mean().values[0]:.3f}")
...
随机森林是一种集成学习方法，通过组合多个决策树来提升预测性能：

- **Bagging（自助采样）**：每个树使用不同的训练子集
- **特征随机性**：每个分裂时随机选择部分特征
- **优势**：减少过拟合，提高泛化能力
...
## 3. GBDT（LightGBM）模型训练与超参数调优

**GBDT（梯度提升决策树）简介**：

GBDT 是一种强大的集成学习方法，通过逐步修正前一轮的错误来提升性能：

- **Boosting（提升）**：弱学习器逐步改进
- **梯度下降**：使用梯度信息优化损失函数
- **LightGBM 优势**：速度快、内存效率高，支持大规模数据

**主要超参数**：

- **n_estimators**：提升轮数
- **learning_rate**：学习率（控制每轮的影响）
- **max_depth**：树的最大深度
- **num_leaves**：叶子节点数量（LightGBM 特有）
- **min_child_samples**：叶节点的最小样本数

**超参数调优策略**：

参考 `lab03_titanic.py`，我们使用 GridSearchCV 进行网格搜索：

- **n_estimators**：树的数量
- **learning_rate**：学习率（控制每棵树的影响）
- **max_depth**：树的最大深度
- **num_leaves**：叶子节点数量（LightGBM 特有）
- **min_child_samples**：叶节点的最小样本数

```{python}
# GBDT（LightGBM）：超参数调优（使用 GridSearchCV）
print("\n" + "=" * 60)
print("GBDT (LightGBM) 超参数调优")
print("=" * 60)

# 定义参数网格（参考 lab03_titanic.py 中的 XGBoost 调参）
param_grid_gbm = {
    'n_estimators': [50, 100, 200],
    'learning_rate': [0.01, 0.05, 0.1, 0.2],
    'max_depth': [3, 5, 7],
    'num_leaves': [15, 31, 63],  # LightGBM 特有参数
...
print(f"\n✅ GBDT 最佳参数: {gbm_grid.best_params_}")
print(f"✅ 最佳交叉验证 AUC: {gbm_grid.best_score_:.4f}")
...
print(f"\n📊 GBDT 测试集性能:")
print(f"  准确率: {gbm_accuracy:.4f}")
print(f"  精确率: {gbm_precision:.4f}")
print(f"  召回率: {gbm_recall:.4f}")
print(f"  F1分数: {gbm_f1:.4f}")
...
plt.xlabel('特征重要性', fontsize=12)
plt.title('GBDT (LightGBM) 特征重要性排序', fontsize=14)
plt.gca().invert_yaxis()  # 最重要的在顶部
plt.grid(axis='x', alpha=0.3)
plt.tight_layout()
plt.show()
...
## 4. 三模型对比：决策树 vs 随机森林 vs GBDT

**对比说明**：

我们将比较三个模型的性能：

1. **决策树**：来自 W3 的训练结果（或重新评估）
2. **随机森林**：使用 GridSearchCV 找到的最佳参数
3. **GBDT (LightGBM)**：使用 GridSearchCV 找到的最佳参数
...
if dt_model_w4 is not None and dt_results_w4 is not None:
    ...
    print("✅ 使用 W4 保存的决策树结果")
else:
    # 重新评估决策树（使用 W4 的最佳参数或默认参数）
    print("⚠️  重新训练决策树...")
...
# 创建对比表格
comparison_data = {
    '模型': ['决策树', '随机森林', 'GBDT (LightGBM)'],
    '准确率': [dt_accuracy, rf_accuracy, gbm_accuracy],
    '精确率': [dt_precision, rf_precision, gbm_precision],
    '召回率': [dt_recall, rf_recall, gbm_recall],
    'F1分数': [dt_f1, rf_f1, gbm_f1],
...
for i, metric in enumerate(metrics):
    ...
    axes[row, col].set_title(f'{metric} 对比', fontsize=12, fontweight='bold')
...
# 最后一个子图：AUC 对比（突出显示）
axes[1, 2].bar(comparison_df['模型'], comparison_df['AUC'], color=colors)
axes[1, 2].set_title('AUC 对比（核心指标）', fontsize=12, fontweight='bold')
...
best_model = comparison_df.loc[best_auc_idx, '模型']
print(f"\n🏆 最佳模型（按 AUC）: {best_model}")
...
plt.plot(fpr_dt, tpr_dt, color='#3498db', linewidth=2, label=f'决策树 (AUC = {auc_dt:.4f})')
plt.plot(fpr_rf, tpr_rf, color='#2ecc71', linewidth=2, label=f'随机森林 (AUC = {auc_rf:.4f})')
...
plt.title('三模型 ROC 曲线对比', fontsize=14, fontweight='bold')
...
print("\nROC 曲线对比分析:")
print("- AUC 越接近 1.0，模型性能越好")
print("- 曲线越靠近左上角，模型性能越好")
print(f"- 最佳模型: {best_model} (AUC = {comparison_df.loc[best_auc_idx, 'AUC']:.4f})")
```
...
1. **集成学习方法**
   - **Bagging（装袋）**：随机森林通过自助采样减少方差
   - **Boosting（提升）**：GBDT 通过逐步修正错误提升性能
   - **特征随机性**：随机森林在特征选择上引入随机性
...
3. **超参数调优**
   - **GridSearchCV**：系统地搜索最优参数组合
   - **交叉验证**：确保参数选择的可靠性
   - **多指标评估**：综合考虑准确率、AUC 等指标
...
- **减少过拟合**：通过组合多个模型提高泛化能力
- **提升性能**：集成方法通常比单一模型表现更好
- **鲁棒性强**：对异常值和噪声有更好的抵抗力

**实战经验总结：**

- ✅ **集成方法通常更强大**：随机森林和 GBDT 往往优于单一决策树
- ✅ **GridSearchCV 很实用**：系统地搜索最优参数，避免手动调参的盲目性
- ✅ **AUC 作为核心指标**：在分类问题中，AUC 比准确率更可靠
- ✅ **特征重要性有价值**：帮助理解模型决策逻辑，支持特征工程
- ✅ **计算效率很重要**：LightGBM 在大规模数据上表现优秀
- ✅ **模型选择要因地制宜**：根据数据规模和业务需求选择合适的算法
