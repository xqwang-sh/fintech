...
1.  **数据划分**：将数据分为训练集和测试集（评估模型泛化能力）。
2.  **特征标准化**：对于逻辑回归等基于距离或梯度的模型，特征缩放非常重要（决策树不需要）。
...
# 将标准化后的数据转回 DataFrame 以保留列名 (方便后续查看)
X_train_scaled = pd.DataFrame(X_train_scaled, columns=X.columns)
X_test_scaled = pd.DataFrame(X_test_scaled, columns=X.columns)

print("特征标准化完成（已生成 X_train_scaled, X_test_scaled）")
```

## 逻辑回归模型实践

我们先尝试一个简单的线性模型——逻辑回归，作为一个 Baseline（基准）。
...
print("\n分类报告:")
print(classification_report(y_test, y_pred_lr, target_names=['死亡', '存活']))
...
plt.title('逻辑回归特征系数 (Feature Coefficients)')
plt.xlabel('系数权重')
...
#| message: false
#| warning: false
# 注意：我们这里使用未标准化的 X_train, y_train，因为决策树对特征尺度不敏感
# 数据集已经在前面划分好了
